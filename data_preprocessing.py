...
def get_motion_data(
    ...
    instrument_motion_data = motion_data[:, -(instrument_marker_num*3):]  # 最後幾個marker是樂器的
    ...
    motion_resample = resample_2d_array(  # downsample from 120fps to 40fps
        motion_data, 
...
def get_snippet(
    ...
    # get stride list
    n_frame_window = int(snippet_time_window * sampling_rate) # 16 * 40
...
def get_song_data_label_snippet_list(
    ...
    # 1. read annot files and pack to dict (note, beat, downbeat, phrase)
    ...
    # 2. convert score position to time position
    #   ( beats & bars -> seconds )
    ...
    song_motion_data = get_motion_data(
        song_motion_file,   # filename
        song_annot_list, 
        motion_original_sampling_rate = 120,
        final_sampling_rate = data_sampling_rate, # 40
        body_marker_num = body_marker_num,  # 30
        instrument_marker_num = instrument_marker_num  # 4
    )
            ...
            for song_name in os.listdir(subset_dir):
                ...
                for perf_name in os.listdir(song_folder):
                    ...
                    for trial_name in os.listdir(perf_folder):
                        ...
                        print('-----')
                        print('Data name: ', song_id_name)  # yv_ba1_yv10_t1
                        ...
                        if song_id_name in problem_files:
                            ...
                            print(f'subset_dir = {subset_dir}, song_name = {song_name}, perf_name = {perf_name}, trial_name = {trial_name}, audio_range = {audio_normalize_range}') 
                            # print(f'song_name = {song_name}')
                            # print(f'perf_name = {perf_name}')
                            # print(f'trial_name = {trial_name}')
                            # print(f'audio_range = {audio_normalize_range}')
                            ...
                            if song_count % train_test_split_ratio == 0:
                                ...
                                song_snippet = get_song_data_label_snippet_list(
                                    subset_dir,  # ~/sinica/lychen/sample_data/yv
                                    song_name,   # ba1
                                    perf_name,   # yv10
                                    trial_name,  # t1
                                    audio_range = audio_normalize_range,  # (0, 30)
...
if __name__ == '__main__':
    ...
    # ===== define data folder =====
    script_path = os.path.abspath(__file__)  # ~/sinica/lychen/data_preprocessing.py
    script_folder = os.path.abspath(os.path.join(script_path, os.pardir)) # ~/sinica/lychen
    # parent_folder = os.path.abspath(os.path.join(script_folder, os.pardir))
    data_folder = os.path.join(script_folder, dataset_name)
    print('Input data from folder:', data_folder)


    # ===== define parameters for data processing =====
    data_sampling_rate = 40  # 40 frame per second
    ...
    snippet_time_window = 16  # 16 seconds per window
    snippet_hop_size = 2
